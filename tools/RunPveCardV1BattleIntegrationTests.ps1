...
$LogFile = Join-Path $LogDir "pve-card-v1-battle-integration-tests.log"
$ScenePath = "res://scenes/pve/TestPveCardV1BattleIntegration.tscn"
...
foreach ($lineObject in $rawOutput) {
    ...
    if ($line -match "\[TestPveCardV1BattleIntegration\] ERROR") {
        $hasTestError = $true
    }
