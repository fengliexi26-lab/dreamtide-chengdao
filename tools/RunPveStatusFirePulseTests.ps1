...
$LogFile = Join-Path $LogDir "pve-status-fire-pulse-tests.log"
$ScenePath = "res://scenes/pve/TestPveStatusFirePulse.tscn"
...
foreach ($lineObject in $rawOutput) {
    ...
    if ($line -match "\[TestPveStatusFirePulse\] ERROR") {
        $hasTestError = $true
    }
